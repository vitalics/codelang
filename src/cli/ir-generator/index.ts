...
const ABORT_SIGNAL_TY     = '%AbortSignal*';
const ABORT_CONTROLLER_TY = '%AbortController*';

function isAsyncTy(ty: string): boolean {
    return ty === TASK_TY || ty === CONTEXT_TY || ty === SHARED_TY ||
           ty === SCHEDULER_TY || ty === ASYNC_TY ||
           ty === ABORT_SIGNAL_TY || ty === ABORT_CONTROLLER_TY;
}
...
const ASYNC_DECLS = [
    ...
    '%Async = type opaque',
    '%AbortSignal = type opaque',
    '%AbortController = type opaque',
].join('\n');

// ── Stream runtime (stdlib/stream.code) ──────────────────────────────────────

const MEMORY_STREAM_TY  = '%MemoryStream*';
const STRING_READER_TY  = '%StringReader*';

function isStreamTy(ty: string): boolean {
    return ty === MEMORY_STREAM_TY || ty === STRING_READER_TY;
}

const STREAM_DECLS = [
    '%MemoryStream = type opaque',
    '%StringReader = type opaque',
].join('\n');

// ── Network runtime (stdlib/network/*.code) ───────────────────────────────────

const TCP_STREAM_TY    = '%TcpStream*';
const TCP_LISTENER_TY  = '%TcpListener*';
const DNS_TY           = '%Dns*';
const UDP_SOCKET_TY    = '%UdpSocket*';
const HTTP_HEADERS_TY  = '%HttpHeaders*';
const HTTP_REQUEST_TY  = '%HttpRequest*';
const HTTP_RESPONSE_TY = '%HttpResponse*';
const HTTP_CLIENT_TY   = '%HttpClient*';
const WEB_SOCKET_TY    = '%WebSocket*';
const HTTP2_CLIENT_TY  = '%Http2Client*';
const HTTP2_RESPONSE_TY= '%Http2Response*';
const HTTP3_CLIENT_TY  = '%Http3Client*';
const HTTP3_RESPONSE_TY= '%Http3Response*';
const URI_TY              = '%Uri*';
const URI_SEARCH_PARAMS_TY = '%UriSearchParams*';

const NET_TYPES = new Set([
    TCP_STREAM_TY, TCP_LISTENER_TY,
    DNS_TY,
    UDP_SOCKET_TY,
    HTTP_HEADERS_TY, HTTP_REQUEST_TY, HTTP_RESPONSE_TY, HTTP_CLIENT_TY,
    WEB_SOCKET_TY,
    HTTP2_CLIENT_TY, HTTP2_RESPONSE_TY,
    HTTP3_CLIENT_TY, HTTP3_RESPONSE_TY,
    URI_TY, URI_SEARCH_PARAMS_TY,
]);

function isNetworkTy(ty: string): boolean {
    return NET_TYPES.has(ty);
}

const NETWORK_DECLS = [
    '%TcpStream    = type opaque',
    '%TcpListener  = type opaque',
    '%Dns          = type opaque',
    '%UdpSocket    = type opaque',
    '%HttpHeaders  = type opaque',
    '%HttpRequest  = type opaque',
    '%HttpResponse = type opaque',
    '%HttpClient   = type opaque',
    '%WebSocket    = type opaque',
    '%Http2Client  = type opaque',
    '%Http2Response = type opaque',
    '%Http3Client  = type opaque',
    '%Http3Response = type opaque',
    '%Uri              = type opaque',
    '%UriSearchParams  = type opaque',
].join('\n');

// ── Date/time runtime (stdlib/date.code) ──────────────────────────────────────

const INSTANT_TY          = '%Instant*';
const PLAIN_DATE_TY       = '%PlainDate*';
const PLAIN_TIME_TY       = '%PlainTime*';
const PLAIN_DATE_TIME_TY  = '%PlainDateTime*';
const ZONED_DATE_TIME_TY  = '%ZonedDateTime*';
const DURATION_TY         = '%Duration*';
const DATE_TIME_FORMAT_TY = '%DateTimeFormat*';
const NOW_TY              = '%Now*';

const DATE_TYPES = new Set([
    INSTANT_TY, PLAIN_DATE_TY, PLAIN_TIME_TY, PLAIN_DATE_TIME_TY,
    ZONED_DATE_TIME_TY, DURATION_TY, DATE_TIME_FORMAT_TY, NOW_TY,
]);

function isDateTy(ty: string): boolean {
    return DATE_TYPES.has(ty);
}

const DATE_DECLS = [
    '%Instant         = type opaque',
    '%PlainDate       = type opaque',
    '%PlainTime       = type opaque',
    '%PlainDateTime   = type opaque',
    '%ZonedDateTime   = type opaque',
    '%Duration        = type opaque',
    '%DateTimeFormat  = type opaque',
    '%Now             = type opaque',
].join('\n');

const BUILD_TY      = '%Build*';
const EXECUTABLE_TY = '%Executable*';
const RUNSTEP_TY    = '%RunStep*';
const STEP_TY       = '%Step*';

const BUILD_TYPES = new Set([BUILD_TY, EXECUTABLE_TY, RUNSTEP_TY, STEP_TY]);

function isBuildTy(ty: string): boolean {
    return BUILD_TYPES.has(ty);
}

/** Opaque type declarations for stdlib/build.code (Build, Executable, RunStep, Step). */
const BUILD_DECLS = [
    '%Build      = type opaque',
...
/**
 * Returns the LLVM storage type for a field inside %Atomic_<suffix>.
 * bool (i1) is widened to i8: LLVM requires naturally-aligned atomic
 * accesses and i1 has no natural atomic size on any target.
 * Returns null for types not supported as atomic values.
 */
function atomicStorageFromSuffix(suffix: string): string | null {
    switch (suffix) {
        case 'i32':  return 'i32';
        case 'i64':  return 'i64';
...
export function generateLLVMIR(modules: ResolvedModule[], sourceFile: string, opts: { debug?: boolean; sourceFilePath?: string } = {}): string {
    ...
    return new GeneratorContext(sourceFile, fnTable, nsTable, externTable, extTable, staticTable, genericExtIndex, staticPropsTable, protocolDefaultsTable, protocolStaticDefaults, fnValExtIndex, opts).generate(modules);
}
...
class GeneratorContext {
    ...
    private usesStream          = false;   // stdlib/stream.code — MemoryStream/StringReader
    private usesNetwork         = false;   // stdlib/network/*.code — TcpStream/HttpClient/WebSocket/…
    private usesDate            = false;   // stdlib/date.code — Instant/PlainDate/ZonedDateTime/…
    private usesBuild           = false;   // stdlib/build.code — Build/Executable/RunStep/Step
    ...
    /** DWARF debug info emitter — non-null only when debug mode is on. */
    private debugInfo: DebugInfo | null = null;
    /** Metadata index of the DISubprogram for the function currently being emitted. */
    private currentDbgScope: number | null = null;
    /** True when at least one @llvm.dbg.declare call was emitted (triggers declare stub). */
    private needsDbgDeclare = false;

    /**
     * LLVM linkage for non-exported user-defined functions.
     *
     * - Release builds: `private` — no symbol emitted, smallest binary.
     * - Debug builds:   `internal` — local symbol is emitted and preserved in the
     *   object file so that dsymutil (macOS) and LLDB can associate DWARF
     *   subprogram entries with the actual function body.  Using `private` in debug
     *   mode causes breakpoints inside non-exported functions (like recursive helpers)
     *   to silently fail because the symbol→address mapping is lost after linking.
     */
    private get privateLinkage(): string {
        return this.debugInfo ? 'internal ' : 'private ';
    }
    ...
    /**
     * Buffer for `alloca` instructions collected during function-body emission.
     * Non-null only while emitStatements runs for a top-level function/method body.
     * At the end of body emission the buffered allocas are spliced into the entry
     * block so that LLVM sees every alloca exactly once on all control-flow paths
     * (required for mem2reg and avoids stack growth inside loops).
     */
    private entryAllocas: string[] | null = null;
    ...
    constructor(
        ...
        debugOpts: { debug?: boolean; sourceFilePath?: string }                  = {},
    ) {
        ...
        if (debugOpts.debug && debugOpts.sourceFilePath) {
            this.debugInfo = new DebugInfo(debugOpts.sourceFilePath);
        }
    }
    ...
    generate(modules: ResolvedModule[]): string {
        ...
        // ── Stream (stdlib/stream.code) ───────────────────────────────────────────
        streamSearch: for (const mod of modules) {
            for (const elem of mod.program.elements) {
                if (isExternDeclaration(elem)) {
                    if (isStreamTy(resolveTypeRef(elem.returnType))) { this.usesStream = true; break streamSearch; }
                    for (const p of elem.parameters)
                        if (isStreamTy(resolveParamType(p))) { this.usesStream = true; break streamSearch; }
                }
                if (isFunctionDeclaration(elem)) {
                    const fn = elem as FunctionDeclaration;
                    if (fn.returnType && isStreamTy(resolveTypeRef(fn.returnType))) { this.usesStream = true; break streamSearch; }
                    for (const p of fn.parameters)
                        if (isStreamTy(resolveParamType(p))) { this.usesStream = true; break streamSearch; }
                }
                if (isExtensionDeclaration(elem)) {
                    for (const method of (elem as ExtensionDeclaration).methods) {
                        if (method.returnType && isStreamTy(resolveTypeRef(method.returnType))) {
                            this.usesStream = true; break streamSearch;
                        }
                        for (const p of method.parameters)
                            if (isStreamTy(resolveParamType(p))) { this.usesStream = true; break streamSearch; }
                    }
        ...
        // ── Network (stdlib/network/*.code) ──────────────────────────────────────
        networkSearch: for (const mod of modules) {
            for (const elem of mod.program.elements) {
                if (isExternDeclaration(elem)) {
                    if (isNetworkTy(resolveTypeRef(elem.returnType))) { this.usesNetwork = true; break networkSearch; }
                    for (const p of elem.parameters)
                        if (isNetworkTy(resolveParamType(p))) { this.usesNetwork = true; break networkSearch; }
                }
                if (isFunctionDeclaration(elem)) {
                    const fn = elem as FunctionDeclaration;
                    if (fn.returnType && isNetworkTy(resolveTypeRef(fn.returnType))) { this.usesNetwork = true; break networkSearch; }
                    for (const p of fn.parameters)
                        if (isNetworkTy(resolveParamType(p))) { this.usesNetwork = true; break networkSearch; }
                }
                if (isExtensionDeclaration(elem)) {
                    for (const method of (elem as ExtensionDeclaration).methods) {
                        if (method.returnType && isNetworkTy(resolveTypeRef(method.returnType))) {
                            this.usesNetwork = true; break networkSearch;
                        }
                        for (const p of method.parameters)
                            if (isNetworkTy(resolveParamType(p))) { this.usesNetwork = true; break networkSearch; }
                    }
        ...
        // ── Date/time (stdlib/date.code) ──────────────────────────────────────────
        dateSearch: for (const mod of modules) {
            for (const elem of mod.program.elements) {
                if (isExternDeclaration(elem)) {
                    if (isDateTy(resolveTypeRef(elem.returnType))) { this.usesDate = true; break dateSearch; }
                    for (const p of elem.parameters)
                        if (isDateTy(resolveParamType(p))) { this.usesDate = true; break dateSearch; }
                }
                if (isFunctionDeclaration(elem)) {
                    const fn = elem as FunctionDeclaration;
                    if (fn.returnType && isDateTy(resolveTypeRef(fn.returnType))) { this.usesDate = true; break dateSearch; }
                    for (const p of fn.parameters)
                        if (isDateTy(resolveParamType(p))) { this.usesDate = true; break dateSearch; }
                }
                if (isExtensionDeclaration(elem)) {
                    for (const method of (elem as ExtensionDeclaration).methods) {
                        if (method.returnType && isDateTy(resolveTypeRef(method.returnType))) {
                            this.usesDate = true; break dateSearch;
                        }
                        for (const p of method.parameters)
                            if (isDateTy(resolveParamType(p))) { this.usesDate = true; break dateSearch; }
                    }
        ...
        // ── Build system (stdlib/build.code) ─────────────────────────────────────
        buildSearch: for (const mod of modules) {
            for (const elem of mod.program.elements) {
                if (isExternDeclaration(elem)) {
                    if (isBuildTy(resolveTypeRef(elem.returnType))) { this.usesBuild = true; break buildSearch; }
                    for (const p of elem.parameters)
                        if (isBuildTy(resolveParamType(p))) { this.usesBuild = true; break buildSearch; }
                }
                if (isFunctionDeclaration(elem)) {
                    const fn = elem as FunctionDeclaration;
                    if (fn.returnType && isBuildTy(resolveTypeRef(fn.returnType))) { this.usesBuild = true; break buildSearch; }
                    for (const p of fn.parameters)
                        if (isBuildTy(resolveParamType(p))) { this.usesBuild = true; break buildSearch; }
                }
                if (isExtensionDeclaration(elem)) {
                    for (const method of (elem as ExtensionDeclaration).methods) {
                        if (method.returnType && isBuildTy(resolveTypeRef(method.returnType))) {
                            this.usesBuild = true; break buildSearch;
                        }
                        for (const p of method.parameters)
                            if (isBuildTy(resolveParamType(p))) { this.usesBuild = true; break buildSearch; }
                    }
        ...
        if ((this.usesMalloc || this.structFieldMap.size > 0 || this.enumDeclMap.size > 0 || this.enumInstantiations.size > 0)
            ...
            header.push('declare void @free(i8*)');
            header.push('');
        }
        ...
        if (this.usesStream) {
            header.push(STREAM_DECLS);
            header.push('');
        }
        if (this.usesNetwork) {
            header.push(NETWORK_DECLS);
            header.push('');
        }
        if (this.usesDate) {
            header.push(DATE_DECLS);
            header.push('');
        }
        if (this.usesBuild) {
            header.push(BUILD_DECLS);
            header.push('');
        }

        // Type declarations for generic instantiations.
        // Atomic<T> gets a concrete struct layout so LLVM can emit
        // atomic load/store/rmw instructions on the first field.
        // All other generic types remain opaque.
        for (const opaqueDecl of this.genericOpaqueDecls) {
            const baseName = opaqueDecl.replace(/^%/, '').replace(/\*$/, '');
            if (baseName.startsWith('Atomic_')) {
                const suffix  = baseName.slice('Atomic_'.length);
                const fieldTy = atomicStorageFromSuffix(suffix);
                if (fieldTy) {
                    header.push(`%${baseName} = type { ${fieldTy} }`);
                    header.push('');
                    continue;
                }
            }
        ...
        //      → we need `%Stacktrace = type opaque` before that struct def.
        {
            ...
            const definedNames = new Set<string>([
                ...
                'AbortSignal', 'AbortController',                        // stdlib/async.code
                'MemoryStream', 'StringReader',                          // stdlib/stream.code
            ]);
        ...
        // ── Debug metadata (DWARF) ────────────────────────────────────────────
        if (this.debugInfo) {
            if (this.needsDbgDeclare) {
                header.push('declare void @llvm.dbg.declare(metadata, metadata, metadata) nounwind readnone');
                header.push('');
            }
            const dbgLines = this.debugInfo.emit();
            return [...header, ...funcs, ...dbgLines].join('\n');
        }
    ...
    private collectStringsInStmt(stmt: Statement): void {
        ...
        if (isMemberCallMemberCallStatement(stmt)) {
            const s = stmt as MemberCallMemberCallStatement;
            for (const a of s.firstArgs) this.collectStringsInCallArg(a);
            for (const a of s.args) this.collectStringsInCallArg(a);
        }
        if (isCallMemberCallStatement(stmt)) {
            const s = stmt as CallMemberCallStatement;
            for (const a of s.callArgs) this.collectStringsInCallArg(a);
            for (const a of s.args) this.collectStringsInCallArg(a);
        }
        if (isCallMemberMemberCallStatement(stmt)) {
            const s = stmt as CallMemberMemberCallStatement;
            for (const a of s.callArgs) this.collectStringsInCallArg(a);
            for (const a of s.firstArgs) this.collectStringsInCallArg(a);
            for (const a of s.args) this.collectStringsInCallArg(a);
        }
    ...
    private collectStringsInExpr(expr: Expression): void {
        ...
        if (isMemberCallMemberCallExpr(expr)) {
            const e = expr as MemberCallMemberCallExpr;
            for (const a of e.firstArgs) this.collectStringsInCallArg(a);
            for (const a of e.args) this.collectStringsInCallArg(a);
        }
        if (isCallMemberCallExpr(expr)) {
            const e = expr as CallMemberCallExpr;
            for (const a of e.callArgs) this.collectStringsInCallArg(a);
            for (const a of e.args) this.collectStringsInCallArg(a);
        }
        if (isCallMemberMemberCallExpr(expr)) {
            const e = expr as CallMemberMemberCallExpr;
            for (const a of e.callArgs) this.collectStringsInCallArg(a);
            for (const a of e.firstArgs) this.collectStringsInCallArg(a);
            for (const a of e.args) this.collectStringsInCallArg(a);
        }
    ...
    private collectStructInfo(modules: ResolvedModule[]): void {
        ...
        // ── Detect callable extension types ──────────────────────────────────
        // An opaque/alias type (no StructBody) can still be made callable by
        // declaring `export static fn call(…): T { … }` inside its
        // `TypeName extends { … }` block.  Scan all ExtensionDeclarations and
        // register matching type names in `callableStructs` so the
        // `TypeName(args)` shorthand desugars to `@TypeName_call(args)`.
        for (const mod of modules) {
            for (const elem of mod.program.elements) {
                if (!isExtensionDeclaration(elem)) continue;
                const extDecl  = elem as ExtensionDeclaration;
                const typeName = extDecl.typeName?.ref?.name;
                if (!typeName) continue;
                // Already registered via StructBody scan above — skip.
                if (this.callableStructs.has(typeName)) continue;
                for (const method of extDecl.methods) {
                    if (method.static && method.name === 'call') {
                        this.callableStructs.add(typeName);
                        break;
                    }
                }
            }
    ...
    private emitFunction(
        ...
        const linkage = nameOverride
            ? this.privateLinkage
            : (isMain || exportedNames.has(fn.name)) ? '' : this.privateLinkage;
        ...
        lines.push(kind);
        if (this.debugInfo) {
            const startLine = (fn.$cstNode?.range.start.line ?? 0) + 1;
            const spIdx = this.debugInfo.subprogram(fn.name, startLine);
            this.currentDbgScope = spIdx;
            lines.push(`define ${linkage}${toLLVM(retTy)} @${llvmName}(${paramList})${attrSuffix} !dbg !${spIdx} {`);
        } else {
        ...
        const entryAllocaInsert = lines.length;
        ...
        if (this.entryAllocas.length > 0)
            lines.splice(entryAllocaInsert, 0, ...this.entryAllocas);
    ...
    private emitExtensionMethod(
        ...
        const linkage    = isExported ? '' : this.privateLinkage;
    ...
    private emitStatement(lines: string[], stmt: Statement, varCtx: VarCtx): boolean {
        ...
        else if (isCallMemberMemberCallStatement(stmt)) { this.emitCallMemberMemberCall(lines, stmt as CallMemberMemberCallStatement, varCtx, false); }
        else if (isCallMemberCallStatement(stmt))        { this.emitCallMemberCall(lines, stmt as CallMemberCallStatement, varCtx, false); }
        else if (isMemberCallMemberCallStatement(stmt))  { this.emitMemberCallMemberCall(lines, stmt as MemberCallMemberCallStatement, varCtx, false); }
    ...
    private emitVarDecl(lines: string[], decl: VariableDeclaration, varCtx: VarCtx): void {
        ...
        // Alloca must live in the function entry block to avoid stack growth in loops.
        // When entryAllocas is non-null we are inside a function body; buffer the
        // instruction so it can be spliced into the entry block after body emission.
        (this.entryAllocas ?? lines).push(`  ${alloca} = alloca ${irTy}, align ${align}`);

        // Emit @llvm.dbg.declare for source-level debuggers (DWARF).
        // Note: we must NOT tag the alloca itself with !dbg — LLDB relies on
        // allocas being location-free for its internal bookkeeping.
        if (this.debugInfo && this.currentDbgScope !== null) {
            const line   = (decl.$cstNode?.range.start.line ?? 0) + 1;
            const col    = (decl.$cstNode?.range.start.character ?? 0) + 1;
            const varIdx = this.debugInfo.localVar(decl.name, line, this.currentDbgScope, irTy);
            const locIdx = this.debugInfo.location(line, col, this.currentDbgScope);
            // Use i8* for i1 allocas to keep the pointer type compatible
            const ptrTy  = irTy === 'i1' ? 'i8' : irTy;
            lines.push(`  call void @llvm.dbg.declare(metadata ${ptrTy}* ${alloca}, metadata !${varIdx}, metadata !DIExpression()), !dbg !${locIdx}`);
            this.needsDbgDeclare = true;
        }
    ...
    private emitMemberCallInstr(
        ...
        if (varInfo) {
            ...
            if (genInfo) {
                // ── Atomic<T> compiler intrinsic — intercept before genericExtIndex ──
                if (genInfo.typeDecl.name === 'Atomic') {
                    return this.emitAtomicMethodIntrinsic(
                        lines, varInfo, member, args, varCtx, capture, genInfo.env,
                    );
                }
        ...
        // ── 2a. Atomic<T>.new() — compiler intrinsic constructor ─────────────────
        // Infers T from the argument type, allocates heap memory, and stores the
        // initial value atomically.  Must be annotated: const x: Atomic<int> = ...
        if (receiver === 'Atomic' && member === 'new') {
            return this.emitAtomicNewIntrinsic(lines, args, varCtx, capture);
        }
    ...
    /**
     * Emit: `(self | ID).firstMember(firstArgs).member(args)`
     *
     * Strategy:
     *   1. Emit `receiver.firstMember(firstArgs)` as a member call → midVal / midTy
     *   2. Alloca a temp slot, store midVal, register in varCtx
     *   3. Dispatch `temp.member(args)` through emitMemberCallInstr
     */
    private emitMemberCallMemberCall(
        lines:   string[],
        node:    MemberCallMemberCallExpr | MemberCallMemberCallStatement,
        varCtx:  VarCtx,
        capture: boolean,
    ): string {
        const receiverName = (node as any).selfCall ? 'self' : ((node as any).namespace ?? '');

        // Step 1: emit receiver.firstMember(firstArgs) → midVal
        const midVal = this.emitMemberCallInstr(lines, receiverName, node.firstMember, node.firstArgs, varCtx, true);
        if (midVal === 'undef') return 'undef';

        // Step 2: infer the type returned by firstMember
        const receiverInfo = varCtx.get(receiverName);
        const midTy = receiverInfo
            ? (this.typeMethodReturnType(receiverInfo.llvmType, node.firstMember) ?? 'i8*')
            : 'i8*';

        if (!midTy || midTy === 'void') {
            lines.push(`  ; WARNING: ${node.firstMember}() returned void — cannot chain .${node.member}()`);
            return 'undef';
        }

        // Step 3: alloca temp slot and register in varCtx
        const tmpName   = `__mc_${this.tmpIdx++}`;
        const tmpAlloca = `%${tmpName}`;
        const irTy      = toLLVM(midTy);
        const align     = alignOf(midTy);
        lines.push(`  ${tmpAlloca} = alloca ${irTy}, align ${align}`);
        lines.push(`  store ${irTy} ${midVal}, ${irTy}* ${tmpAlloca}, align ${align}`);

        const savedEntry = varCtx.get(tmpName);
        varCtx.set(tmpName, { allocaName: tmpAlloca, llvmType: midTy });

        // Step 4: dispatch member(args) on the temp
        const result = this.emitMemberCallInstr(lines, tmpName, node.member, node.args, varCtx, capture);
    ...
    /**
     * Emit: `callee(callArgs).member(args)`
     *
     * Strategy:
     *   1. Emit `callee(callArgs)` → callVal / callRetTy
     *   2. Alloca a temp slot, store callVal, register in varCtx
     *   3. Dispatch `temp.member(args)` through emitMemberCallInstr
     */
    private emitCallMemberCall(
        lines:   string[],
        node:    CallMemberCallExpr | CallMemberCallStatement,
        varCtx:  VarCtx,
        capture: boolean,
    ): string {
        // Step 1: emit callee(callArgs) → callVal
        const callRetTy = this.inferCalleeReturnType(node.callee, varCtx);
        const callVal   = this.emitCallInstr(lines, node.callee, node.callArgs, varCtx, true, false, callRetTy);
        if (callVal === 'undef') return 'undef';

        if (!callRetTy || callRetTy === 'void') {
            lines.push(`  ; WARNING: ${node.callee}() returned void — cannot chain .${node.member}()`);
            return 'undef';
        }

        // Step 2: alloca temp slot
        const tmpName   = `__cmc_${this.tmpIdx++}`;
        const tmpAlloca = `%${tmpName}`;
        const irTy      = toLLVM(callRetTy);
        const align     = alignOf(callRetTy);
        lines.push(`  ${tmpAlloca} = alloca ${irTy}, align ${align}`);
        lines.push(`  store ${irTy} ${callVal}, ${irTy}* ${tmpAlloca}, align ${align}`);

        const savedEntry = varCtx.get(tmpName);
        varCtx.set(tmpName, { allocaName: tmpAlloca, llvmType: callRetTy });

        // Step 3: dispatch member(args)
        const result = this.emitMemberCallInstr(lines, tmpName, node.member, node.args, varCtx, capture);
    ...
    /**
     * Emit: `callee(callArgs).firstMember(firstArgs).member(args)`
     *
     * Strategy:
     *   1. Emit `callee(callArgs)` → r1Val / r1Ty
     *   2. Alloca temp1, store r1Val
     *   3. Dispatch `temp1.firstMember(firstArgs)` → r2Val / r2Ty
     *   4. Alloca temp2, store r2Val
     *   5. Dispatch `temp2.member(args)` → final result
     */
    private emitCallMemberMemberCall(
        lines:   string[],
        node:    CallMemberMemberCallExpr | CallMemberMemberCallStatement,
        varCtx:  VarCtx,
        capture: boolean,
    ): string {
        // Step 1: emit callee(callArgs) → r1Val
        const r1Ty    = this.inferCalleeReturnType(node.callee, varCtx);
        const r1Val   = this.emitCallInstr(lines, node.callee, node.callArgs, varCtx, true, false, r1Ty);
        if (r1Val === 'undef') return 'undef';

        if (!r1Ty || r1Ty === 'void') {
            lines.push(`  ; WARNING: ${node.callee}() returned void — cannot chain .${node.firstMember}()`);
            return 'undef';
        }

        // Step 2: alloca temp1 for the callee result
        const tmp1Name   = `__cmmc1_${this.tmpIdx++}`;
        const tmp1Alloca = `%${tmp1Name}`;
        const irTy1      = toLLVM(r1Ty);
        const align1     = alignOf(r1Ty);
        lines.push(`  ${tmp1Alloca} = alloca ${irTy1}, align ${align1}`);
        lines.push(`  store ${irTy1} ${r1Val}, ${irTy1}* ${tmp1Alloca}, align ${align1}`);

        const saved1 = varCtx.get(tmp1Name);
        varCtx.set(tmp1Name, { allocaName: tmp1Alloca, llvmType: r1Ty });

        // Step 3: emit temp1.firstMember(firstArgs) → r2Val
        const r2Val = this.emitMemberCallInstr(lines, tmp1Name, node.firstMember, node.firstArgs, varCtx, true);

        if (saved1 === undefined) varCtx.delete(tmp1Name);
        else varCtx.set(tmp1Name, saved1);

        if (r2Val === 'undef') return 'undef';

        const r2Ty = this.typeMethodReturnType(r1Ty, node.firstMember) ?? 'i8*';
        ...
        // Step 4: alloca temp2 for the firstMember result
        const tmp2Name   = `__cmmc2_${this.tmpIdx++}`;
        const tmp2Alloca = `%${tmp2Name}`;
        const irTy2      = toLLVM(r2Ty);
        const align2     = alignOf(r2Ty);
        lines.push(`  ${tmp2Alloca} = alloca ${irTy2}, align ${align2}`);
        lines.push(`  store ${irTy2} ${r2Val}, ${irTy2}* ${tmp2Alloca}, align ${align2}`);

        const saved2 = varCtx.get(tmp2Name);
        varCtx.set(tmp2Name, { allocaName: tmp2Alloca, llvmType: r2Ty });

        // Step 5: dispatch member(args)
        const result = this.emitMemberCallInstr(lines, tmp2Name, node.member, node.args, varCtx, capture);

        if (saved2 === undefined) varCtx.delete(tmp2Name);
        else varCtx.set(tmp2Name, saved2);
    ...
    private emitCallInstr(
        ...
        // ── Callable struct/extension: TypeName(args) → @TypeName_call(args) ──
        // Supported in two forms:
        //   (a) Struct type with a CallableMethod or `static fn call` in its body.
        //   (b) Opaque/alias type with `export static fn call` in its extension block.
        //
        //   type MyType extends Callable<int, int> {
        //     call(n: int): int { return n * 2; }
        //   }
        //   export type Uri = intrinsic("%Uri*");
        //   Uri extends { export static fn call(s: string): Uri { … } }
        //
        //   const x = MyType(500);     →  %x = call i32 @MyType_call(i32 500)
        //   const u = Uri("https://…") →  %u = call %Uri* @Uri_call(i8* %s)
        if (this.callableStructs.has(callee)) {
            // ── Path A: StructBody type ───────────────────────────────────────
            ...
            // ── Path B: Extension-declared `static fn call` ───────────────────
            // Opaque / alias types (e.g. `type Uri = intrinsic(…)`) don't have
            // a StructBody, but their extension block may carry `static fn call`.
            const callEntry = this.staticTable.get(callee)?.get('call');
            if (callEntry) {
                const method   = callEntry.method;
                const retTy    = method.returnType ? resolveTypeRefWithEnv(method.returnType, EMPTY_ENV) : 'void';
                const retIrTy  = toLLVM(retTy);
                const argParts: string[] = [];
                for (let i = 0; i < args.length; i++) {
                    const p      = method.parameters[i];
                    const pTy    = p?.type ? resolveTypeRefWithEnv(p.type, EMPTY_ENV) : undefined;
                    const pIrTy  = pTy ? toLLVM(pTy) : 'i8*';
                    const aReg   = this.emitExpr(lines, args[i], varCtx, pTy ?? 'i8*');
                    argParts.push(`${pIrTy} ${aReg}`);
                }
                const mangledName = `${callee}_call`;
                const argStr      = argParts.join(', ');
    ...
    // ── Atomic<T> compiler intrinsics ─────────────────────────────────────────
    //
    // Atomic<T> operations are lowered to LLVM atomic instructions inline —
    // no C runtime calls, no generic specialization queue.
    //
    // Layout: %Atomic_i32 = type { i32 }   (%Atomic_bool uses i8, etc.)
    // Pointer alignment for %Atomic_*: 8 bytes (pointer-sized alloca).
    // Field alignment: natural alignment of the storage type.

    /**
     * Atomic.new(val: T) — allocate heap memory and store initial value.
     * T is inferred from the argument.  Requires an explicit type annotation
     * on the receiving variable:  const x: Atomic<int> = Atomic.new(0).
     */
    private emitAtomicNewIntrinsic(
        lines:   string[],
        args:    Expression[],
        varCtx:  VarCtx,
        capture: boolean,
        ...
        if (args.length !== 1) {
            lines.push('  ; ERROR: Atomic.new requires exactly one argument');
            return 'undef';
        }

        const argTy = this.inferType(args[0], varCtx);
        const suffix = llvmTypeToSuffix(argTy);
        const fieldTy = atomicStorageFromSuffix(suffix);
        if (!fieldTy) {
            lines.push(`  ; ERROR: Atomic.new: unsupported type '${argTy}'`);
            return 'undef';
        }

        const structTy  = `%Atomic_${suffix}`;
        const fieldAlign = alignOf(fieldTy);

        // Emit the initial value; bool must be widened to i8.
        let storeVal = this.emitExpr(lines, args[0], varCtx, argTy, false);
        let storeTy  = fieldTy;
        if (argTy === 'i1') {
            const ext = `%${this.tmpIdx++}`;
            lines.push(`  ${ext} = zext i1 ${storeVal} to i8`);
            storeVal = ext;
        }

        // Allocate: sizeof(%Atomic_T) bytes via malloc.
        this.usesMalloc = true;
        const szPtr  = `%${this.tmpIdx++}`;
        const szInt  = `%${this.tmpIdx++}`;
        const rawPtr = `%${this.tmpIdx++}`;
        const typed  = `%${this.tmpIdx++}`;
        lines.push(`  ${szPtr}  = getelementptr ${structTy}, ${structTy}* null, i32 1`);
        lines.push(`  ${szInt}  = ptrtoint ${structTy}* ${szPtr} to i64`);
        lines.push(`  ${rawPtr} = call i8* @malloc(i64 ${szInt})`);
        lines.push(`  ${typed}  = bitcast i8* ${rawPtr} to ${structTy}*`);

        // Store initial value through the first (and only) field.
        const fldPtr = `%${this.tmpIdx++}`;
        lines.push(`  ${fldPtr} = getelementptr inbounds ${structTy}, ${structTy}* ${typed}, i32 0, i32 0`);
        lines.push(`  store ${storeTy} ${storeVal}, ${storeTy}* ${fldPtr}, align ${fieldAlign}`);

        // Ensure the type is registered so the header emits %Atomic_* = type { ... }.
        this.genericOpaqueDecls.add(`${structTy}*`);

        return capture ? typed : '';
    }

    /**
     * Emit an Atomic<T> instance-method intrinsic (load, store, add, sub, cas, free).
     * All operations use seq_cst memory ordering.
     */
    private emitAtomicMethodIntrinsic(
        lines:   string[],
        varInfo: { allocaName: string; llvmType: string },
        member:  string,
        args:    Expression[],
        varCtx:  VarCtx,
        capture: boolean,
        ...
        const T = typeEnv.get('T');
        if (!T) {
            lines.push(`  ; ERROR: Atomic.${member}: could not resolve T`);
            return 'undef';
        }

        const suffix    = llvmTypeToSuffix(T);
        const fieldTy   = atomicStorageFromSuffix(suffix);
        if (!fieldTy) {
            lines.push(`  ; ERROR: Atomic.${member}: unsupported T='${T}'`);
            return 'undef';
        }
        ...
        const selfVal = `%${this.tmpIdx++}`;
        lines.push(`  ${selfVal} = load ${structTy}*, ${structTy}** ${varInfo.allocaName}, align 8`);

        // GEP to the first (and only) field.
        const fldPtr = `%${this.tmpIdx++}`;
        lines.push(`  ${fldPtr} = getelementptr inbounds ${structTy}, ${structTy}* ${selfVal}, i32 0, i32 0`);

        switch (member) {

            case 'load': {
                const raw = `%${this.tmpIdx++}`;
                lines.push(`  ${raw} = load atomic ${fieldTy}, ${fieldTy}* ${fldPtr} seq_cst, align ${fieldAlign}`);
                if (T === 'i1') {
                    const r = `%${this.tmpIdx++}`;
                    lines.push(`  ${r} = trunc i8 ${raw} to i1`);
                    return capture ? r : '';
                }
                return capture ? raw : '';
            }
            ...
            case 'store': {
                let sVal = this.emitExpr(lines, args[0], varCtx, T, false);
                let sTy  = fieldTy;
                if (T === 'i1') {
                    const ext = `%${this.tmpIdx++}`;
                    lines.push(`  ${ext} = zext i1 ${sVal} to i8`);
                    sVal = ext;
                    sTy  = 'i8';
                }
                lines.push(`  store atomic ${sTy} ${sVal}, ${sTy}* ${fldPtr} seq_cst, align ${fieldAlign}`);
                return '';
            }
            ...
            case 'sub': {
                if (T !== 'i32' && T !== 'i64') {
                    lines.push(`  ; ERROR: Atomic.${member} requires int or int64`);
                    return 'undef';
                }
                const delta  = this.emitExpr(lines, args[0], varCtx, T, false);
                const op     = member === 'add' ? 'add' : 'sub';
                const result = `%${this.tmpIdx++}`;
                lines.push(`  ${result} = atomicrmw ${op} ${fieldTy}* ${fldPtr}, ${fieldTy} ${delta} seq_cst`);
                return capture ? result : '';
            }

            case 'cas': {
                // Compare-and-swap: returns bool (i1) — true if swap succeeded.
                let expVal = this.emitExpr(lines, args[0], varCtx, T, false);
                let desVal = this.emitExpr(lines, args[1], varCtx, T, false);
                let casFieldTy  = fieldTy;
                let casFieldPtr = fldPtr;

                if (T === 'i1') {
                    // bool: widen to i8 for cmpxchg
                    const e8 = `%${this.tmpIdx++}`;
                    const d8 = `%${this.tmpIdx++}`;
                    lines.push(`  ${e8} = zext i1 ${expVal} to i8`);
                    lines.push(`  ${d8} = zext i1 ${desVal} to i8`);
                    expVal = e8;
                    desVal = d8;
                    casFieldTy = 'i8';
                } else if (T === 'float') {
                    // float: bitcast to i32 then use i32 cmpxchg
                    const eBits = `%${this.tmpIdx++}`;
                    const dBits = `%${this.tmpIdx++}`;
                    const iPtr  = `%${this.tmpIdx++}`;
                    lines.push(`  ${eBits}  = bitcast float ${expVal} to i32`);
                    lines.push(`  ${dBits}  = bitcast float ${desVal} to i32`);
                    lines.push(`  ${iPtr}   = bitcast float* ${fldPtr} to i32*`);
                    expVal = eBits;
                    desVal = dBits;
                    casFieldTy  = 'i32';
                    casFieldPtr = iPtr;
                } else if (T === 'double') {
                    // double: bitcast to i64 then use i64 cmpxchg
                    const eBits = `%${this.tmpIdx++}`;
                    const dBits = `%${this.tmpIdx++}`;
                    const iPtr  = `%${this.tmpIdx++}`;
                    lines.push(`  ${eBits}  = bitcast double ${expVal} to i64`);
                    lines.push(`  ${dBits}  = bitcast double ${desVal} to i64`);
                    lines.push(`  ${iPtr}   = bitcast double* ${fldPtr} to i64*`);
                    expVal = eBits;
                    desVal = dBits;
                    casFieldTy  = 'i64';
                    casFieldPtr = iPtr;
                ...
                const cxRes   = `%${this.tmpIdx++}`;
                const success = `%${this.tmpIdx++}`;
                lines.push(`  ${cxRes}   = cmpxchg ${casFieldTy}* ${casFieldPtr}, ${casFieldTy} ${expVal}, ${casFieldTy} ${desVal} seq_cst seq_cst`);
                lines.push(`  ${success} = extractvalue { ${casFieldTy}, i1 } ${cxRes}, 1`);
                return capture ? success : '';
            }

            case 'free': {
                const asI8 = `%${this.tmpIdx++}`;
                lines.push(`  ${asI8} = bitcast ${structTy}* ${selfVal} to i8*`);
                lines.push(`  call void @free(i8* ${asI8})`);
                this.usesMalloc = true;   // ensures @malloc/@free are declared
                return '';
            }

            default:
                lines.push(`  ; ERROR: unknown Atomic method '${member}'`);
                return 'undef';
        }
    }
    ...
    private emitExpr(
        ...
        if (isCallMemberMemberCallExpr(expr)) {
            return this.emitCallMemberMemberCall(lines, expr as CallMemberMemberCallExpr, varCtx, true);
        }

        if (isCallMemberCallExpr(expr)) {
            return this.emitCallMemberCall(lines, expr as CallMemberCallExpr, varCtx, true);
        }

        if (isMemberCallMemberCallExpr(expr)) {
            return this.emitMemberCallMemberCall(lines, expr as MemberCallMemberCallExpr, varCtx, true);
        }
    ...
    private inferType(expr: Expression, varCtx: VarCtx): string {
        ...
        if (isMemberCallMemberCallExpr(expr)) {
            // Infer: receiver.firstMember(firstArgs) → midTy, then midTy.member(args) → returnType
            const mce = expr as MemberCallMemberCallExpr;
            const receiverName = mce.selfCall ? 'self' : (mce.namespace ?? '');
            const receiverInfo = varCtx.get(receiverName);
            let midTy: string | null | undefined;
            if (!receiverInfo) {
                // Receiver is a static namespace (e.g. OS, GPU) — not a local variable.
                // Look up the first call as a static method on that namespace.
                const staticEntry = this.staticTable.get(receiverName)?.get(mce.firstMember);
                if (staticEntry?.method.returnType) {
                    midTy = resolveTypeRef(staticEntry.method.returnType);
                } else {
                    return 'i8*';
                }
            } else {
                midTy = this.typeMethodReturnType(receiverInfo.llvmType, mce.firstMember);
            }
            if (!midTy || midTy === 'void') return 'i8*';
            return this.typeMethodReturnType(midTy, mce.member) ?? 'i8*';
        }
        if (isCallMemberCallExpr(expr)) {
            // Infer: callee(callArgs) → callRetTy, then callRetTy.member(args) → returnType
            const cce = expr as CallMemberCallExpr;
            const callRetTy = this.inferCalleeReturnType(cce.callee, varCtx);
            if (!callRetTy || callRetTy === 'void') return 'i8*';
            return this.typeMethodReturnType(callRetTy, cce.member) ?? 'i8*';
        }
        if (isCallMemberMemberCallExpr(expr)) {
            // Infer: callee(callArgs) → r1Ty, r1Ty.firstMember(firstArgs) → r2Ty, r2Ty.member(args) → returnType
            const cce = expr as CallMemberMemberCallExpr;
            const r1Ty = this.inferCalleeReturnType(cce.callee, varCtx);
            if (!r1Ty || r1Ty === 'void') return 'i8*';
            const r2Ty = this.typeMethodReturnType(r1Ty, cce.firstMember);
            if (!r2Ty || r2Ty === 'void') return 'i8*';
            return this.typeMethodReturnType(r2Ty, cce.member) ?? 'i8*';
        }
        if (isPostfixCallExpr(expr)) {
            ...
            if (genInfo) {
                ...
                // Atomic<T> compiler intrinsic — return types are known statically
                if (genInfo.typeDecl.name === 'Atomic') {
                    const T = genInfo.env.get('T');
                    if (T) {
                        switch (pce.member) {
                            case 'load': return T;
                            case 'add':
        ...
        if (isCallExpression(expr)) {
            ...
            // Callable struct/extension: TypeName(args) → return type of TypeName.call
            if (this.callableStructs.has(callee)) {
                // Path A: StructBody type
                ...
                // Path B: Extension-declared `static fn call` (opaque/alias types)
                const callEntry = this.staticTable.get(callee)?.get('call');
                if (callEntry?.method.returnType) {
                    const ret = resolveTypeRefWithEnv(callEntry.method.returnType, EMPTY_ENV);
                    return ret === 'void' ? 'i8*' : ret;
                }
            }
        }
    ...
    /**
     * Infer the return type of a simple callee name (function/extern/callable struct).
     * Used by the chaining emitters to determine the type of an intermediate result.
     * Returns 'i8*' as the fallback (unknown but non-void).
     */
    private inferCalleeReturnType(callee: string, varCtx: VarCtx): string {
        const fn = this.fnTable.get(callee);
        if (fn) {
            const ret = this.resolveReturnType(fn, this.prePassVarCtx(fn));
            return (!ret || ret === 'void') ? 'i8*' : ret;
        ...
        // Static callable struct / extension: TypeName(args) → return type of call()
        const callEntry = this.staticTable.get(callee)?.get('call');
        if (callEntry?.method.returnType) {
            const ret = resolveTypeRefWithEnv(callEntry.method.returnType, EMPTY_ENV);
            return (!ret || ret === 'void') ? 'i8*' : ret;
        }
        // Auto-generated constructor: TypeName(...) → %TypeName*
        if (this.structFieldMap.has(callee)) return `%${callee}*`;
        // Callable struct (direct call syntax)
        if (this.callableStructs.has(callee)) {
            const td   = this.structTypeDecls.get(callee);
            const body = td?.body as StructBody | undefined;
            const cm   = body?.members.find(m =>
    ...
    private emitGenericFunctionSpec(
        ...
        const linkage = exportedNames.has(fn.name) ? '' : this.privateLinkage;
    ...
    private collectCaptures(
        ...
        const scanExpr = (e: Expression) => {
            ...
            if (isMemberCallMemberCallExpr(e)) {
                const mc = e as MemberCallMemberCallExpr;
                const ns = mc.namespace as string | undefined;
                if (ns && !ownLocals.has(ns) && outerCtx.has(ns) && !captured.has(ns)) {
                    const info = outerCtx.get(ns)!;
                    captured.set(ns, { llvmType: info.llvmType, allocaName: info.allocaName });
                }
                for (const a of mc.firstArgs) scanExpr(a);
                for (const a of mc.args) scanExpr(a);
            }
            if (isCallMemberCallExpr(e)) {
                const cc = e as CallMemberCallExpr;
                if (!ownLocals.has(cc.callee) && outerCtx.has(cc.callee) && !captured.has(cc.callee)) {
                    const info = outerCtx.get(cc.callee)!;
                    captured.set(cc.callee, { llvmType: info.llvmType, allocaName: info.allocaName });
                }
                for (const a of cc.callArgs) scanExpr(a);
                for (const a of cc.args) scanExpr(a);
            }
            if (isCallMemberMemberCallExpr(e)) {
                const cc = e as CallMemberMemberCallExpr;
                if (!ownLocals.has(cc.callee) && outerCtx.has(cc.callee) && !captured.has(cc.callee)) {
                    const info = outerCtx.get(cc.callee)!;
                    captured.set(cc.callee, { llvmType: info.llvmType, allocaName: info.allocaName });
                }
                for (const a of cc.callArgs) scanExpr(a);
                for (const a of cc.firstArgs) scanExpr(a);
                for (const a of cc.args) scanExpr(a);
            }
        ...
        // Walk a Condition node (BinaryCondition | BoolExprCondition) for captures.
        const scanCondition = (c: Condition) => {
            if (!c) return;
            if (isBinaryCondition(c)) {
                scanCondition((c as BinaryCondition).left);
                scanCondition((c as BinaryCondition).right);
            } else {
                // BoolExprCondition — wraps a plain Expression
                const expr = (c as BoolExprCondition).expr;
                if (expr) scanExpr(expr);
            }
        };
        ...
        const scanStmt = (s: Statement) => {
            ...
            if (isMemberCallMemberCallStatement(s)) {
                const mc = s as MemberCallMemberCallStatement;
                const ns = mc.namespace as string | undefined;
                if (ns && !ownLocals.has(ns) && outerCtx.has(ns) && !captured.has(ns)) {
                    const info = outerCtx.get(ns)!;
                    captured.set(ns, { llvmType: info.llvmType, allocaName: info.allocaName });
            ...
            if (isCallMemberCallStatement(s)) {
                const cc = s as CallMemberCallStatement;
                if (!ownLocals.has(cc.callee) && outerCtx.has(cc.callee) && !captured.has(cc.callee)) {
                    const info = outerCtx.get(cc.callee)!;
                    captured.set(cc.callee, { llvmType: info.llvmType, allocaName: info.allocaName });
                }
            ...
            if (isCallMemberMemberCallStatement(s)) {
                const cc = s as CallMemberMemberCallStatement;
                if (!ownLocals.has(cc.callee) && outerCtx.has(cc.callee) && !captured.has(cc.callee)) {
                    const info = outerCtx.get(cc.callee)!;
                    captured.set(cc.callee, { llvmType: info.llvmType, allocaName: info.allocaName });
                }
            ...
            if (isIfStatement(s)) {
                scanCondition((s as IfStatement).condition);
            ...
            if (isWhileStatement(s)) {
                scanCondition((s as WhileStatement).condition);
                for (const ss of (s as WhileStatement).body.statements) scanStmt(ss);
            }
            if (isForStatement(s)) {
                if ((s as ForStatement).init.value) scanExpr((s as ForStatement).init.value!);
                if ((s as ForStatement).condition)  scanCondition((s as ForStatement).condition!);
                if ((s as ForStatement).update?.value) scanExpr((s as ForStatement).update!.value!);
                for (const ss of (s as ForStatement).body.statements) scanStmt(ss);
            }
        };
    ...
    private emitLambdaExpr(
        ...
        const emitLambdaBody = (lLines: string[], innerVarCtx: VarCtx) => {
            ...
            const entryAllocaInsert = lLines.length;
            const terminated = this.emitStatements(lLines, expr.body.statements, innerVarCtx);

            if (this.entryAllocas.length > 0)
                lLines.splice(entryAllocaInsert, 0, ...this.entryAllocas);
